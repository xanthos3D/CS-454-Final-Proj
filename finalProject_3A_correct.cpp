 ...
 mpz_class fiveOrBelow(int size){
        ...
        //for(int length = 1; length <= size; length++ ){
            ...
            if(length != 1){
                ...
                // loop based on the number of combinations for that length
                for (int combs = 0; combs < pow(length, 4); ++combs) {

                    //so we make a empty string and a int
                    std::string buffer;
                    int temp = combs;

                    //generates our string of correct length
                    ...
                    //finds the midpoint of that string
                    int half = buffer.length()/2 ;

                    //if the string has a a in the middle
                    if(buffer[half] == 'a'){

                        //std::cout<<buffer<<std::endl;

                        //increment our return variable.
